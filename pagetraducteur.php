...
<?php
    ...
    <form action="pagetraducteur-processing.php" method="post">
        Mot à ajouter en base de donnée : <br>
        <input type="text" name="mot" value="<?= $_SESSION['mot1'] ?>">
        Langue : <select name="lang" id="">
            <option value="francais" <?= ($_SESSION['selected'] == 'fr' ? 'selected' : '' )  ?>> Francais </option>
            <option value="anglais" <?= ($_SESSION['selected'] == 'en' ? 'selected' : '' )  ?>> Anglais </option>
        </select><br>
        <input type="text" name="trad"><br>
...
while($dbRow = mysqli_fetch_assoc($dbResult)){
    ...
    if(empty($dbRow['fr']))
        echo 'Demande de traduction du mot anglais ' . $dbRow['en'] . ' en français  ';
    else
        echo 'Demande de traduction du mot français ' . $dbRow['fr'] . ' en anglais  ';
